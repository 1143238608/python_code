...
def click_point_change():
    click_point = "185,49|89,213"
    click_point_array = click_point.split('|')
    result = [(int(x), int(y)) for x, y in [item.split(',') for item in click_point_array]]
    generated_click = []
    for x, y in result:
        x_generated, y_generated = generate_values_from_real(x, y)
        generated_click.append(str(x_generated)+'_'+str(y_generated))
    print(','.join(generated_click))
    return ','.join(generated_click)
