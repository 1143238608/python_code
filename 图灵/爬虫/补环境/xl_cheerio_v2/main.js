...
// const jscode = fs.readFileSync(`${run_path}/ths123.js`)
const jscode = fs.readFileSync(`${run_path}/boss.js`)
// const jscode = fs.readFileSync(`${run_path}/ths123.js`)
...
// debugger
    vm.setGlobal('bobocheerio', cheerio)
    vm.setGlobal('bocreateCanvas', createCanvas)
    const codeTest = `${configCode}${log_code}${toolsCode}${envCode}${globadlThis}${globalInit}${userInit}${proxyObj};;${jscode}${asyncCode};console.table(myloglist);debugger;function boss(){return (new window.ABC).z(${seed}, ${ts})}boss()`
    const script = new VMScript(codeTest, "./debugJS.js")
    const result = vm.run(script);
    return result
    // console.log(result)
